import { WindFarmParams } from "./types.js";
...
export const DEFAULT_WIND_FARM_PARAMS: WindFarmParams = {
  turbineCount: 6,
  ratedPowerKw: 900,
  cutInSpeedMps: 3,
  ratedSpeedMps: 12,
...
export function configuredInputTimeZone(fallback = "UTC"): string {
...
export function getStoredInputTimeZone(): string | null {
  const stored = localStorage.getItem(INPUT_TIMEZONE_STORAGE_KEY)?.trim();
  if (!stored) return null;
  return isValidTimeZone(stored) ? stored : null;
}

export function saveInputTimeZone(zone: string): void {
  localStorage.setItem(INPUT_TIMEZONE_STORAGE_KEY, zone);
}

export function clearInputTimeZone(): void {
  localStorage.removeItem(INPUT_TIMEZONE_STORAGE_KEY);
}

export function validateWindFarmParams(params: WindFarmParams): boolean {
  if (!Number.isFinite(params.turbineCount) || params.turbineCount <= 0 || !Number.isInteger(params.turbineCount)) return false;
  if (!Number.isFinite(params.ratedPowerKw) || params.ratedPowerKw <= 0) return false;
  if (!Number.isFinite(params.cutInSpeedMps) || params.cutInSpeedMps < 0) return false;
  if (!Number.isFinite(params.ratedSpeedMps) || params.ratedSpeedMps <= params.cutInSpeedMps) return false;
  if (!Number.isFinite(params.cutOutSpeedMps) || params.cutOutSpeedMps <= params.ratedSpeedMps) return false;
  if (!Number.isFinite(params.referenceAirDensityKgM3) || params.referenceAirDensityKgM3 <= 0) return false;
  if (!Number.isFinite(params.minOperatingTempC) || !Number.isFinite(params.maxOperatingTempC)) return false;
  if (params.minOperatingTempC >= params.maxOperatingTempC) return false;
  if (!Number.isFinite(params.minOperatingPressureHpa) || !Number.isFinite(params.maxOperatingPressureHpa)) return false;
  if (params.minOperatingPressureHpa >= params.maxOperatingPressureHpa) return false;
  return true;
}

function parseCandidateWindFarm(raw: string | null): WindFarmParams | null {
  if (!raw) return null;
  try {
    const parsed = JSON.parse(raw) as Partial<WindFarmParams>;
    const defaults = DEFAULT_WIND_FARM_PARAMS;
    const candidate: WindFarmParams = {
...
export function readStoredWindFarmParamsOptional(): WindFarmParams | null {
  const raw = localStorage.getItem(WIND_FARM_STORAGE_KEY);
  return parseCandidateWindFarm(raw);
}

export function readStoredWindFarmParams(defaultParams = DEFAULT_WIND_FARM_PARAMS): WindFarmParams {
  const parsed = readStoredWindFarmParamsOptional();
  if (parsed) return parsed;
  return { ...defaultParams };
}

export function saveWindFarmParams(params: WindFarmParams): void {
  localStorage.setItem(WIND_FARM_STORAGE_KEY, JSON.stringify(params));
}

export function clearWindFarmParams(): void {
