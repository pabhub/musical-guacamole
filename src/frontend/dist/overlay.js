...
export function createOverlayController(map) {
    return {
...
export function stationColor(stationId) {
    if (stationId.toUpperCase().endsWith("R"))
        return "#0284c7";
    if (stationId === "89070")
        return "#ea580c";
    return "#0f766e";
}
export function renderStationMarkers(map, controller, snapshots, profiles, selectedStationId, onStationClick) {
    controller.markerLayer.clearLayers();
    controller.markers.clear();
    controller.selectedLatLng = null;
    const profileMap = new Map(profiles.map((profile) => [profile.stationId, profile]));
    const bounds = [];
    for (const snapshot of snapshots) {
        ...
        marker.bindPopup(`<strong>${snapshot.stationName}</strong><br/>Latest: ${formatDateTime(snapshot.datetime)}<br/>Wind: ${formatNumber(snapshot.speed)} m/s`);
...
function arrowHead(start, end, size = 0.0035) {
    const [y1, x1] = start;
    const [y2, x2] = end;
    const vx = x2 - x1;
    const vy = y2 - y1;
    ...
    const left = [tailY + py * size, tailX + px * size];
    const right = [tailY - py * size, tailX - px * size];
    return [end, left, right];
}
export function renderPlaybackOverlay(controller, frame, options) {
    controller.vectorLayer.clearLayers();
    controller.trailLayer.clearLayers();
    controller.climateLayer.clearLayers();
    if (!frame || !controller.selectedLatLng)
        return;
    ...
    if (options.showTemperatureHalo && frame.temperature != null) {
        ...
        L.circle([lat, lon], { radius, color: haloColor, weight: 1.5, opacity: 0.48, fillOpacity: 0.1 }).addTo(controller.climateLayer);
    }
    if (options.showPressureRing && frame.pressure != null) {
        ...
        L.circle([lat, lon], { radius, color: "#7c3aed", weight: 1.2, opacity: 0.4, fillOpacity: 0 }).addTo(controller.climateLayer);
    }
}
export function clearPlaybackTrail(controller) {
