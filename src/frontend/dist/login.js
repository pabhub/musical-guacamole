...
renderLoginPage();
const form = requiredElement("login-form");
const usernameInput = requiredElement("login-username");
const passwordInput = requiredElement("login-password");
const submitButton = requiredElement("login-submit");
const statusEl = requiredElement("login-status");
...
form.addEventListener("submit", async (event) => {
    ...
    try {
        const payload = await fetchJson("/api/auth/token", {
            method: "POST",
