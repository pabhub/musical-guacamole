...
function extractYearsFromPayload(payload) {
    const years = new Set();
...
export class TimeframeManager {
    ...
    async fetchTimeframePayload(stationId, range, simulation, forceRefreshOnEmpty = false) {
        const params = timeframeQueryParams(range, stationId, this.deps.configuredInputTimeZone(), simulation);
        if (forceRefreshOnEmpty)
            params.set("forceRefreshOnEmpty", "true");
        return fetchJson(`/api/analysis/timeframes?${params.toString()}`);
    }
    mergedYears(payload) {
        const payloadYears = extractYearsFromPayload(payload);
        const merged = new Set([...this.availableYears, ...payloadYears]);
    ...
    async loadAnalytics() {
        ...
        try {
            ...
            this.deps.setTimeframeStatus(`Timeframe analysis ready: ${payload.buckets.length} buckets across ${years.length} loaded year(s), grouped by ${payload.groupBy}.`, "ok");
