...
function snapshotAggregationForHistoryYears(years) {
    if (years <= 2)
        return "hourly";
    return "daily";
}
function selectedStationLatestDate(ctx, stationId) {
    return selectedStationLatestDateFromHistory(ctx.state.bootstrapState, stationId);
}
function computeBaselineStart(ctx, stationId, years) {
    return computeBaselineStartFromHistory(ctx.state.bootstrapState, stationId, years, toDateTimeLocalInZone, ctx.configuredInputTimeZone());
}
async function pollQueryJob(ctx, jobId) {
    while (true) {
        const status = await fetchJson(`/api/analysis/query-jobs/${jobId}`);
        ctx.setQueryProgress(status);
        if (status.status === "failed")
            throw new Error(status.errorDetail ?? "Query job failed.");
        if (status.status === "complete")
            return status;
        await new Promise((resolve) => window.setTimeout(resolve, 2000));
    }
}
export async function handleMapStationClick(ctx, stationIdClicked) {
    ...
    if (!ctx.state.bootstrapState.selectableStationIds.includes(primaryId))
        return;
    const years = ctx.selectedHistoryYears();
    const alreadyLoaded = (primaryId === ctx.state.lastLoadedStationId &&
    ...
    renderStationMarkers(ctx.map, ctx.overlayController, ctx.state.bootstrapState.latestSnapshots, ctx.state.bootstrapState.stations, primaryId, ctx.getMapStationClickHandler());
    if (alreadyLoaded) {
...
export async function runAnalysisJob(ctx) {
    if (!ctx.ensureAuthenticated())
        return;
    if (!ctx.state.bootstrapState)
        return;
    ...
    const stationName = stationDisplayName(station, ctx.state.bootstrapState.stations.find((item) => item.stationId === station)?.stationName);
    ...
    if (!resolvedLatest) {
        try {
            const availability = await fetchJson(`/api/metadata/latest-availability/station/${encodeURIComponent(station)}`);
            if (ctx.state.bootstrapState) {
    ...
    ctx.setLoading(true);
    try {
        const created = await fetchJson("/api/analysis/query-jobs", {
            method: "POST",
        ...
        const snapshot = await fetchJson(`/api/analysis/query-jobs/${created.jobId}/result`);
