...
function populateStations(response, stationSelect) {
...
export async function bootstrapDashboard(ctx) {
    if (!ctx.ensureAuthenticated())
        return;
    if (ctx.state.dashboardBootstrapped)
        return;
    ctx.setError(null);
    ...
    ctx.setLoading(true);
    try {
        const bootstrap = await fetchJson("/api/analysis/bootstrap");
        ctx.state.bootstrapState = bootstrap;
        populateStations(bootstrap, ctx.elements.stationSelect);
        ctx.elements.stationSelect.value = "";
        renderStationMarkers(ctx.map, ctx.overlayController, bootstrap.latestSnapshots, bootstrap.stations, null, ctx.getMapStationClickHandler());
