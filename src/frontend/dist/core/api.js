...
function tokenExpiryFromStorage() {
...
function writeLastActivityMs(nowMs) {
    localStorage.setItem(authLastActivityStorageKey, String(nowMs));
}
function isInactivityExpired(nowMs) {
...
export function recordAuthActivity(force = false) {
    ...
    if (!force && nowMs - lastActivityWriteMs < activityWriteThrottleMs)
        return;
...
async function refreshAuthTokenIfNeeded() {
    ...
    if (lastActivityMs == null)
        return false;
    if (nowMs - lastActivityMs > authRecentActivityForRefreshMs)
        return false;
    const remainingSeconds = expiry - nowEpochSeconds();
    if (remainingSeconds > authRefreshThresholdSeconds)
        return false;
    if (nowMs - lastRefreshAttemptMs < authRefreshCooldownMs)
        return false;
    if (refreshInFlight)
    ...
    refreshInFlight = (async () => {
        ...
        if (typeof nextToken !== "string" || typeof expiresIn !== "number")
            return false;
...
export function saveAuthToken(token, expiresInSeconds, touchActivity = true) {
...
export function startAuthSessionManager() {
